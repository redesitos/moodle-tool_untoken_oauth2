...
if ( confirm_sesskey() && $confirm && $delete) {
     ...
     $table->head = array('linkid','userid', 'username', 'email','status');
     $deluser = $DB->get_records_sql('SELECT * 
					FROM {auth_oauth2_linked_login} WHERE '.$DB->sql_compare_text('email')
						.' = '.$DB->sql_compare_text(':email'), ['email' => $email]);
     ...
     foreach($deluser as $usr){
        $table->data[] = array(
				...
				$usr->userid,
				$usr->username,
				$usr->email,get_string('deleted','tool_untoken_oauth2')
	);
